...
paths:
  /:
	...
	example := doc.Paths.Value("/").Get.Responses.Get(200).Value.Content.Get("application/json").Examples["test"]
...
func TestLoadWithReferenceInReferenceInProperty(t *testing.T) {
	...
	err = doc.Validate(loader.Context)
	require.NoError(t, err)
	require.Equal(t, "Problem details", doc.Paths.Value("/api/test/ref/in/ref/in/property").Post.Responses["401"].Value.Content["application/json"].Schema.Value.Properties["error"].Value.Title)
}
...
func TestLoadYamlFileWithExternalPathRef(t *testing.T) {
	...
	require.NotNil(t, doc.Paths.Value("/test").Get.Responses["200"].Value.Content["application/json"].Schema.Value.Type)
	require.Equal(t, "string", doc.Paths.Value("/test").Get.Responses["200"].Value.Content["application/json"].Schema.Value.Type)
}
...
`)
	...
	response := doc.Paths.Value(`/users/{id}`).Get.Responses.Get(200).Value
...
func TestLinksFromOAISpec(t *testing.T) {
	...
	err = doc.Validate(loader.Context)
	require.NoError(t, err)
	response := doc.Paths.Value(`/2.0/repositories/{username}/{slug}`).Get.Responses.Get(200).Value
